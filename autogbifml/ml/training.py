    ...
    def save(self, output_path: str):
        if self.algorithm == AlgorithmEnum.DECISION_TREE:
            joblib.dump(self.model,
                        os.path.join(output_path, "decision_tree_model.joblib"))

        elif self.algorithm == AlgorithmEnum.RANDOM_FOREST:
            joblib.dump(self.model,
                        os.path.join(output_path, "random_forest_model.joblib"))

        elif self.algorithm == AlgorithmEnum.XGBOOST:
            self.model.save_model(
                os.path.join(output_path, "xgboost_model.ubj"))
        ...
        with mlflow.start_run(run_name=f"trial-{trial.number}"):
            ...
            for fold_i, (train_idx,
                ...
                mlflow.log_figure(fig,
                                  f"confusion_matrix_fold_{fold_i + 1}.png")
    ...
    def get_param_grid(self, trial: optuna.Trial) -> dict:
        ...
        if self.config.algorithm == AlgorithmEnum.DECISION_TREE:
            return {
                ...
                "max_features":
                    trial.suggest_categorical("max_features", [None, "sqrt"]),
                "min_samples_split":
            ...
            return {
                "max_depth":
                    trial.suggest_int("max_depth", 3, 110),
                "learning_rate":
            ...
            return {
                ...
                "iterations":
                    trial.suggest_int("iterations", 10, 2000, step=10),
                "depth":
