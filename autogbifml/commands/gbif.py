        ...
        with ZipFile(file_path, "r") as zip_ref:
            with zip_ref.open("citations.txt") as citations_file:
                ...
                with zip_ref.open("occurrence.txt") as occurrence_file:
                    ...
                    df["latitude"] = df.apply(GBIFMetaCommand.coalesce_coordinate("decimalLatitude"), axis=1)
                    df["longitude"] = df.apply(GBIFMetaCommand.coalesce_coordinate("decimalLongitude"), axis=1)

                    # subset dataset
                    df_subset = df[["occurrenceID", "ts", "latitude", "longitude", "species"]]
